...
output "idms_config_path" {
  description = "Path to IDMS config file for zero-egress clusters (null if not generated)."
  value       = var.create_ecr && var.zero_egress ? module.ecr[0].idms_config_path : null
}
...
#------------------------------------------------------------------------------
# KMS Information (Always present in GovCloud - FedRAMP requirement)
# Separate keys for blast radius containment
#------------------------------------------------------------------------------

output "cluster_kms_mode" {
  description = "Cluster KMS mode: 'create' or 'existing'."
  value       = var.cluster_kms_mode
}

output "cluster_kms_key_arn" {
  description = "Cluster KMS key ARN for ROSA workers/etcd."
  value       = local.cluster_kms_key_arn
}

output "infra_kms_mode" {
  description = "Infrastructure KMS mode: 'create' or 'existing'."
  value       = var.infra_kms_mode
}

output "infra_kms_key_arn" {
  description = "Infrastructure KMS key ARN for jump host/CloudWatch/S3/VPN."
...
output "jumphost_ssm_command" {
  ...
  value       = var.create_jumphost ? "aws ssm start-session --target ${module.jumphost[0].instance_id} --region ${var.aws_region}" : null
}
...
output "compliance_summary" {
  description = "GovCloud compliance configuration."
  value = {
    fips_enabled         = true
    ...
    infra_kms_key_arn    = local.infra_kms_key_arn
    customer_managed_kms = true # Always true in GovCloud (FedRAMP requirement)
...
output "quickstart_commands" {
  ...
  value       = <<-EOT
    # Connect via SSM jump host (recommended)
...
output "cluster_auth_summary" {
  description = <<-EOT
    ...
    NOTE: GovCloud clusters are always private - establish VPN first.
  EOT
...
output "gitops_status" {
  ...
  value = var.install_gitops ? (
    length(module.gitops) > 0 ? {
      ...
      status  = "skipped"
      message = "GitOps skipped - GovCloud clusters require VPN/jump host connectivity"
    }
    ) : {
    status  = "disabled"
    message = "Set install_gitops = true (after establishing VPN connectivity)"
  }
}
