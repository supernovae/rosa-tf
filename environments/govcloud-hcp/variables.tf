#------------------------------------------------------------------------------
# ROSA HCP - AWS GovCloud Variables
#
# GovCloud Enforcement:
# - FIPS: Always enabled (not configurable)
# - Private cluster: Always enabled (not configurable)
# - KMS encryption: Always required (not configurable)
#------------------------------------------------------------------------------
...
variable "ocm_token" {
  type        = string
  description = <<-EOT
    OCM offline token from https://console.openshiftusgov.com/openshift/token
    
    SECURITY: Never commit this value. Use TF_VAR_ocm_token environment variable.
  EOT
  sensitive   = true
}
...
variable "aws_region" {
  type        = string
  description = "AWS GovCloud region for the cluster."
  default     = "us-gov-west-1"

  validation {
    condition     = contains(["us-gov-west-1", "us-gov-east-1"], var.aws_region)
    error_message = "Must be a valid AWS GovCloud region: us-gov-west-1 or us-gov-east-1."
  }
}
...
variable "channel_group" {
  type        = string
  description = "Update channel: stable, fast, candidate, or eus (recommended for GovCloud)."
  default     = "eus"
...
# Note: private_cluster and fips are NOT configurable in GovCloud
#------------------------------------------------------------------------------
...
# aws/ebs key ARN (arn:aws-us-gov:kms:REGION:ACCOUNT:alias/aws/ebs) at your
# own risk. This is NOT recommended for production or compliance workloads.
#------------------------------------------------------------------------------
...
variable "cluster_kms_mode" {
  ...
  description = <<-EOT
    ...
    - "create" (DEFAULT): Terraform creates customer-managed KMS key
      * Full control over key policies, rotation, and audit
      * Required for FedRAMP SC-12/SC-13 key management controls
    
    - "existing": Use an existing KMS key ARN (set cluster_kms_key_arn)
      * For centralized key management across accounts
      * Key must have appropriate policy for ROSA roles
...
variable "cluster_kms_key_arn" {
  type        = string
  description = <<-EOT
    ARN of existing KMS key for cluster. Required when cluster_kms_mode = "existing".
    
    Key requirements:
    - Must be in the same region as the cluster
    - Must have key policy allowing ROSA account and operator roles
    
    Example: arn:aws-us-gov:kms:us-gov-west-1:123456789012:key/...
  EOT
  default     = null
...
variable "infra_kms_key_arn" {
  type        = string
  description = <<-EOT
    ARN of existing KMS key for infrastructure. Required when infra_kms_mode = "existing".
...
variable "support_role_external_id" {
  type        = string
  description = "External ID for Red Hat support role."
  default     = ""
}
...
#------------------------------------------------------------------------------
# Jump Host Configuration (Recommended for GovCloud private clusters)
#------------------------------------------------------------------------------

variable "create_jumphost" {
  type        = bool
  description = "Create SSM-enabled jump host (recommended for private cluster access)."
  default     = true
}
...
variable "gitops_repo_url" {
  type        = string
  description = "Git repository URL for GitOps manifests (layers, custom apps)."
  default     = null
}
...
variable "layers_install_method" {
  type        = string
  description = <<-EOT
    How to install GitOps layers:
    
    "direct" (default):
      - Applies layers from local manifests via API calls
      - Works in air-gapped environments
      - No external Git access required from cluster
    
    "applicationset":
      - ArgoCD ApplicationSet pulls from your Git repo
      - Requires cluster egress to Git + forked repo
