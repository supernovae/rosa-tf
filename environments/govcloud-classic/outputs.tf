...
#------------------------------------------------------------------------------
# KMS Outputs (Always present in GovCloud - FedRAMP requirement)
...
output "kms_key_summary" {
  ...
  value = {
    ...
    customer_managed    = true # Always true in GovCloud (FedRAMP requirement)
...
output "cluster_auth_summary" {
  description = <<-EOT
    ...
    GovCloud clusters are ALWAYS private. You MUST have network
    connectivity to the cluster VPC to install GitOps. Options:
    - Connect via Client VPN (see vpn_endpoint_id output)
    - Use jump host / bastion within VPC
    - Configure Transit Gateway / Direct Connect
  ...
  value = var.install_gitops ? (
    length(module.gitops) > 0 ? {
      ...
      message = <<-EOT
        ...
        GovCloud clusters are ALWAYS private.
        You MUST establish network connectivity to the cluster VPC:
        
        1. Connect via Client VPN:
           - Download VPN config from vpn_client_configuration output
           - Import into OpenVPN client
           - Connect before re-running Terraform
        
        2. Or use jump host / bastion within VPC
        
        3. Re-run: terraform apply -var-file=<your>.tfvars
