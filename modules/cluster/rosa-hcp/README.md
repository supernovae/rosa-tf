...
`1.7.8-prerelease.2`. Deletion protection defaults on, instance metadata requires
IMDSv2, and native creation-only bootstrap credentials remain sensitive in state.

`current_version` reports the deployed control-plane version. Lifecycle ignores
version changes: changing the requested install version is **not** an upgrade.
